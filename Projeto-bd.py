...
def login():
    ...
    nome_utilizador = args["username"]
    password = hashlib.sha256(args["password"].encode()).hexdigest()
    logger.debug(f'PASS: {password} USER: {nome_utilizador}')
    query = "SELECT password FROM person WHERE nome = %s"
    message = {}
            ...
            with conn.cursor() as cursor:
                ...
                if cursor.rowcount == 0:
                    ...
                    if password == row[0]:
                        ...
                        payload = {'username': nome_utilizador}
